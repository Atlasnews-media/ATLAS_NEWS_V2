...
SCRIPT_PATH = ROOT / "tmp" / "contingency-audio-2026-09-09.txt"
OUTPUT_STEM = "2026-09-09-reading-ipom-escenario-2026"
SOURCE_ID = "2026-09-09-reading-ipom-escenario-2026"
...
def prepare_paragraphs(text: str) -> list[str]:
    ...
    speech_text = " ".join(paragraphs).lower()
    required = (
    ...
    missing = [fragment for fragment in required if fragment not in speech_text]
    if missing:
        raise RuntimeError(
            "Speech Normalizer no produjo las formas esperadas: " + ", ".join(missing)
        )
    return paragraphs
    ...
    for paragraph_index, paragraph in enumerate(paragraphs, start=1):
        ...
        for chunk_index, (_, _, audio) in enumerate(
            ...
            if not np.isfinite(array).all():
                raise RuntimeError(
                    f"Kokoro produjo valores no finitos en párrafo {paragraph_index}, chunk {chunk_index}."
                )
