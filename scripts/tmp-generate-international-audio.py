...
SCRIPT_PATH = ROOT / "tmp" / "international-audio-2026-09-09.txt"
OUTPUT_STEM = "2026-09-09-international-analysis"
SOURCE_ID = "2026-09-09-daily-petroleo-sobre-cien-endurece-dilema-monetario"
...
def synthesize_paragraph(pipeline: KPipeline, paragraph: str) -> np.ndarray:
    ...
    for _, _, audio in pipeline(
        ...
        if array.size:
            chunks.append(array)
    if not chunks:
        raise RuntimeError("Kokoro no devolvió audio para un párrafo de Internacional.")
...
def synthesize(text: str) -> np.ndarray:
    ...
    for index, paragraph in enumerate(paragraphs, start=1):
        ...
        print(f"INTERNACIONAL 09-09 · párrafo {index} · {samples.size / SAMPLE_RATE:.1f}s")

    if not parts:
        raise RuntimeError("El guion de Internacional está vacío.")
...
out_dir = PUBLIC_DIR / "audio" / "international"
...
metadata = {
    "schemaVersion": 1,
    "status": "published",
    "kind": "international-analysis",
    "sourceId": SOURCE_ID,
    "sourceCommit": os.environ.get("ATLAS_INTERNATIONAL_SOURCE_COMMIT"),
    ...
    "path": f"/audio/international/{mp3_path.name}",
