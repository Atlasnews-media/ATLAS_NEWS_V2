...
import { REQUIRED_ARTIFACTS, validateBaselineRequest } from "./validate-baseline-request.mjs";
...
for (const key of REQUIRED_ARTIFACTS) {
  artifacts[key] = key.endsWith(".md") || key.endsWith(".html") ? "fixture " + key : {};
}
artifacts["run.json"] = {
  schemaVersion: 1,
  ...identity,
...
for (const section of ["general", "national", "markets"]) {
  artifacts["checkpoints/" + section + ".json"] = {
    ...
    phase: section,
    status: "PASS",
    completedAt: "2026-09-24T09:0" + (section === "general" ? "1" : section === "national" ? "2" : "3") + ":00-03:00",
  };
}
...
try {
  validateBaselineRequest({ ...request, artifacts: { ...artifacts, "result.json": { sourceCommit: sourceMainCommit } } });
} catch {
  forbiddenRejected = true;
}
if (!forbiddenRejected) throw new Error("campo productivo no fue rechazado");
console.log("INGEST PRODUCT FIELD FORBIDDEN: PASS");

let scopeRejected = false;
try {
  validateBaselineRequest({ ...request, artifacts: { ...artifacts, "../status.json": {} } });
} catch {
  scopeRejected = true;
}
if (!scopeRejected) throw new Error("artifact fuera de contrato no fue rechazado");
...
const active = JSON.parse(await fs.readFile(path.join(temp, "lab/radar-editorial/runtime/active.json"), "utf8"));
if (active.status !== "GENERAL_PASS") throw new Error("materializador no dejó GENERAL_PASS");
if (await fs.stat(path.join(temp, "lab/radar-editorial/runtime/runs", runId, "general/output.md")).then(() => false, () => true)) {
