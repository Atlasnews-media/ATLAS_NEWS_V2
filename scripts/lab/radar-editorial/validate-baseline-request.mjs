...
function walkForbidden(value, where) {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (FORBIDDEN_KEYS.has(key)) fail("campo productivo prohibido: " + where + "." + key);
...
function assertIdentity(value, request, label) {
  for (const key of [
    ...
    if (value[key] !== request[key]) fail(label + "." + key + " no coincide con request");
  }
}

export function validateBaselineRequest(request) {
  if (!request || typeof request !== "object" || Array.isArray(request)) fail("request debe ser objeto");
  assertExactKeys(request, REQUEST_KEYS, "request");
  if (request.schemaVersion !== 1) fail("request.schemaVersion debe ser 1");
  if (!/^[A-Za-z0-9._-]{8,128}$/.test(request.radarRunId || "")) fail("radarRunId inválido");
  if (!/^[0-9a-f]{40}$/.test(request.sourceMainCommit || "")) fail("sourceMainCommit debe ser SHA completo");
  if (!/^radar-state-v[1-9][0-9]*$/.test(request.radarStateVersion || "")) fail("radarStateVersion inválida");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(request.editorialDate || "")) fail("editorialDate inválida");
  if (request.experimentMode !== "BASELINE") fail("V1 sólo acepta experimentMode BASELINE");
  if (request.mode !== "LAB_ONLY") fail("mode debe ser LAB_ONLY");
  if (request.productionWritesAllowed !== false) fail("productionWritesAllowed debe ser false");
  if (request.producer !== "CHATGPT_TASK") fail("producer debe ser CHATGPT_TASK");
  assertIso(request.createdAt, "createdAt");
  if (!request.artifacts || typeof request.artifacts !== "object" || Array.isArray(request.artifacts)) fail("artifacts debe ser objeto");
  ...
  for (const key of REQUIRED_ARTIFACTS) {
    ...
    if (key.endsWith(".md") || key.endsWith(".html")) {
      if (typeof value !== "string" || !value.trim()) fail("artifact textual inválido: " + key);
...
async function main() {
  ...
  console.log("RADAR REQUEST PASS — " + request.radarRunId + " · sha256=" + sha256);
}
