...
function assertIdentity(request) {
  ...
  if (!/^[A-Za-z0-9._:-]{8,128}$/.test(request.radarRunId ?? ""))
    fail("jev-request.radarRunId inválido");
  if (!/^[0-9a-f]{40}$/.test(request.sourceMainCommit ?? ""))
    fail("jev-request.sourceMainCommit inválido");
  if (!/^radar-state-v[1-9][0-9]*$/.test(request.radarStateVersion ?? ""))
    fail("jev-request.radarStateVersion inválido");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(request.editorialDate ?? ""))
    fail("jev-request.editorialDate inválido");
}

function normalizeCandidate(candidate) {
  if (!candidate || typeof candidate !== "object")
    fail("candidateReview inválido");
  if (typeof candidate.candidateKey !== "string" || !candidate.candidateKey)
    fail("candidateReview.candidateKey inválido");
  if (typeof candidate.title !== "string" || !candidate.title.trim())
    fail(`${candidate.candidateKey}: title inválido`);
  if (
    typeof candidate.description !== "string" ||
    candidate.description.length < 20
  )
    fail(`${candidate.candidateKey}: description inválida`);
  if (!Array.isArray(candidate.priors))
    fail(`${candidate.candidateKey}: priors debe ser lista`);
  return {
...
function questions() {
  return {
    event_relation: {
      type: "choice",
      instructions:
        "Judge the relationship between the current candidate event and the supplied historical priors. Choose one option only.",
      criteria: {
        SAME_EVENT:
          "The candidate concerns the same underlying event already represented in the priors.",
        NEW_EVENT:
          "The candidate concerns a materially distinct event, even if topic or entities overlap.",
        UNRELATED:
          "The candidate is not meaningfully related to the supplied priors.",
        UNCERTAIN:
          "The supplied evidence is insufficient to resolve the event relationship.",
      },
    },
    information_relation: {
      type: "choice",
      instructions:
        "Judge the information contribution of the current candidate relative to the supplied priors. Event identity and information contribution are separate judgments.",
      criteria: {
        NEW: "The candidate contributes substantially new information not present in the priors.",
        CONTINUATION:
          "The candidate continues an existing story without a material change in the core facts.",
        MEANINGFUL_UPDATE:
          "The candidate materially changes or advances the known situation.",
        REPETITION:
          "The candidate substantially repeats information already present in the priors.",
        CONTRADICTION:
          "The candidate materially contradicts a prior fact or claim.",
        UNCERTAIN:
          "The supplied evidence is insufficient to resolve the information relationship.",
      },
    },
    adds_new_information: {
      type: "noul",
      instructions:
        "Does the candidate add materially new information relative to the supplied priors?",
    },
    historical_context_needed: {
      type: "noul",
      instructions:
        "Would the candidate be materially easier to understand if the supplied historical context were surfaced?",
    },
    angle_repeated: {
      type: "noul",
      instructions:
        "Does the candidate substantially repeat the same essential editorial angle or thesis used in the supplied priors, rather than merely sharing a topic?",
    },
  };
}
...
async function main() {
  ...
  if (requestIndex < 0 || outputIndex < 0)
    fail("uso: run-jev-memory-judge.mjs --request FILE --output FILE");
  ...
  if (
    !Array.isArray(request.candidateReviews) ||
    request.candidateReviews.length < 1
  ) {
  ...
  for (const candidate of candidates) {
    const result = await client.runQuestions({
      state: {
        ...
        historicalPriors: candidate.priors,
      },
      questions: questions(),
    });
    ...
    if (!result.ok || !result.payload?.answers) {
      fail(
        `JEV no devolvió respuesta válida para ${candidate.candidateKey}: ${JSON.stringify(result.error)}`,
      );
    }

    modelObserved ??=
      typeof result.payload.model === "string" ? result.payload.model : null;
  ...
  console.log(
    `JEV MEMORY PASS — ${request.radarRunId} · ${judgments.length} candidate(s)`,
  );
}

main().catch((error) => {
  console.error(
    `JEV MEMORY FAIL: ${error.code ?? error.name ?? "Error"}: ${error.message}`,
  );
  process.exit(1);
});
