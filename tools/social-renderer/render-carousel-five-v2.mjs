...
const VISUAL_ORDER = ["energy", "fed", "equity", "treasury", "oil", "chile", "port"];
const KEYWORDS = Object.freeze({
  energy: ["hormuz", "saudita", "oleoducto", "energia", "ataque", "ruta alternativa", "mar rojo"],
  oil: ["petroleo", "brent", "wti", "crudo", "barriles", "prima energetica"],
  fed: ["fed", "inflacion", "ipc", "fomc", "tasa", "tasas", "subida"],
  equity: ["wall street", "s&p", "nasdaq", "dow", "acciones", "bolsa", "renta variable"],
  treasury: ["treasury", "bonos", "renta fija", "duracion", "carry", "rendimiento", "curva"],
...
function rankedVisualKeys(text) {
  ...
  const scored = VISUAL_ORDER.map((key, order) => {
    const score = KEYWORDS[key].reduce((total, word) => total + (hay.includes(normalize(word)) ? 1 : 0), 0);
...
function chooseUniqueKeys(texts) {
  ...
  for (const text of texts) {
    ...
    const key = ranked.find((candidate) => !used.has(candidate)) || VISUAL_ORDER.find((candidate) => !used.has(candidate));
...
async function main() {
  ...
  if (new Set(keys).size !== 5) throw new Error(`Carousel visual uniqueness guard failed: ${keys.join(", ")}`);
  const [coverVisual, ideaVisual, changedVisual, impactVisual, closingVisual] = await Promise.all(keys.map(loadVisual));
  ...
  const slides = [
    ...
    `${imageLayer(closingVisual, 135, 70, 620, .84)}`,
  ];
