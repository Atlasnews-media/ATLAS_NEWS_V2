...
const CAROUSEL_FILES = Array.from({ length: 5 }, (_, i) => `atlas-news-instagram-carousel-0${i + 1}.png`);
...
async function waitForContainer(containerId, label) {
  ...
  throw new Error(`${label} container ${containerId} did not become ready in time`);
}

async function assertPublicImages(imageUrls) {
  for (const url of imageUrls) {
    const parsed = new URL(url);
    if (parsed.protocol !== 'https:') throw new Error(`Image URL must use https: ${url}`);
    let ok = false;
    ...
    if (!ok) throw new Error(`Public image is not fetchable as an image: ${url}`);
  }
}
...
async function main() {
  ...
  if (PUBLISH_CONFIRMATION !== REQUIRED_PUBLISH_CONFIRMATION) {
    throw new Error(`Instagram publish guard failed: expected "${REQUIRED_PUBLISH_CONFIRMATION}"`);
  }
  ...
  const sourceId = String(contract.sourceId || contract.sourceCommit || '').trim();
  if (!sourceId) throw new Error('contract.sourceId or contract.sourceCommit is required');
  const canonical = new URL(canonicalUrl);
  if (canonical.protocol !== 'https:') throw new Error('contract.canonicalUrl must use https');
  ...
  const recent = await api(`/${encodeURIComponent(igUserId)}/media?fields=id,caption,permalink,timestamp&limit=25`);
  const duplicate = Array.isArray(recent.data)
    ? recent.data.find((item) => String(item.caption || '').includes(canonicalUrl))
    : null;
  if (duplicate) {
    throw new Error(`Anti-duplicate guard: this canonical URL is already present in Instagram media ${duplicate.id}`);
  }
  ...
  try {
    verified = await api(`/${encodeURIComponent(mediaId)}?fields=id,permalink,media_type,timestamp,caption`);
  } catch (error) {
    console.warn(`Post-publish verification metadata unavailable: ${error.message}`);
  }
  ...
  await fs.writeFile(path.join(OUT, 'instagram-publish.json'), JSON.stringify(result, null, 2));
