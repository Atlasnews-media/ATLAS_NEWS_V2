...
const MARK = 'data:image/png;base64,' + (await fs.readFile(path.resolve(process.cwd(), 'assets/atlas-mark-mini.base64'), 'utf8')).trim();
const PAPER = '#f2efe5';
const NAVY = '#0a3554';
const BLUE = '#1b5d93';
...
async function main(){
  ...
  const og=await generateOpenGraphImage({title:' ',description:'',bgImage:{path:cardPre,fit:'fill'},padding:1,fonts:[],font:{title:{size:1,color:[242,239,229]},description:{size:1,color:[242,239,229]}},cacheDir:path.join(TMP,'og-cache')});
  ...
  const tow=performance.now(); await generateOpenGraphImage({title:' ',description:'',bgImage:{path:cardPre,fit:'fill'},padding:1,fonts:[],font:{title:{size:1,color:[242,239,229]},description:{size:1,color:[242,239,229]}},cacheDir:path.join(TMP,'og-cache')}); const ogCacheHitMs=performance.now()-tow;
